...
func main() {
	...
	var score int
	for pos := range seen {
		if pos[0] == 0 || pos[0] == len(input)-1 || pos[1] == 0 || pos[1] == len(input[0])-1 {
			continue
		}
		t := input[pos[0]][pos[1]]
		if input[pos[0]+1][pos[1]] >= t || input[pos[0]-1][pos[1]] >= t || input[pos[0]][pos[1]+1] >= t || input[pos[0]][pos[1]-1] >= t {
			continue
		}
		var a, b, c, d int
		for i, j := pos[0]+1, pos[1]; i < len(input); i++ {
			a++
			if input[i][j] >= t {
				break
			}
		}
		for i, j := pos[0]-1, pos[1]; i >= 0; i-- {
			b++
			if input[i][j] >= t {
				break
			}
		}
		for i, j := pos[0], pos[1]+1; j < len(input[0]); j++ {
			c++
			if input[i][j] >= t {
				break
			}
		}
		for i, j := pos[0], pos[1]-1; j >= 0; j-- {
			d++
			if input[i][j] >= t {
				break
			}
