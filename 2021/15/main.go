...
func main() {
	riskMap := make([][]int, len(input))
	for i := range riskMap {
		riskMap[i] = make([]int, len(input[0]))
		for j := range riskMap[i] {
			riskMap[i][j] = math.MaxInt
		}
	}
	riskMap[0][0] = 0
	queue := make([][2]int, 0, 100*100)
	queue = append(queue, [2]int{0, 0})
	for len(queue) > 0 {
		point := queue[0]
		queue = queue[1:]
		i, j := point[0], point[1]
		if i > 0 && riskMap[i-1][j] > riskMap[i][j]+input[i-1][j] {
			riskMap[i-1][j] = riskMap[i][j] + input[i-1][j]
			queue = append(queue, [2]int{i - 1, j})
		}
		if i < len(input)-1 && riskMap[i+1][j] > riskMap[i][j]+input[i+1][j] {
			riskMap[i+1][j] = riskMap[i][j] + input[i+1][j]
			queue = append(queue, [2]int{i + 1, j})
		}
		if j > 0 && riskMap[i][j-1] > riskMap[i][j]+input[i][j-1] {
			riskMap[i][j-1] = riskMap[i][j] + input[i][j-1]
			queue = append(queue, [2]int{i, j - 1})
		}
		if j < len(input[0])-1 && riskMap[i][j+1] > riskMap[i][j]+input[i][j+1] {
			riskMap[i][j+1] = riskMap[i][j] + input[i][j+1]
			queue = append(queue, [2]int{i, j + 1})
		}
	}
	fmt.Println(riskMap[len(riskMap)-1][len(riskMap[0])-1])

	input2 := make([][]int, len(input)*5)
	for i := range input2 {
		input2[i] = make([]int, len(input[0])*5)
		for j := range input2[i] {
			input2[i][j] = input[i%len(input)][j%len(input[0])] + i/len(input) + j/len(input[0])
			if input2[i][j] != 9 {
				input2[i][j] %= 9
			}
		}
	}
	riskMap2 := make([][]int, len(input)*5)
	for i := range riskMap2 {
		riskMap2[i] = make([]int, len(input[0])*5)
		for j := range riskMap2[i] {
			riskMap2[i][j] = math.MaxInt
		}
	}

	riskMap2[0][0] = 0
	queue2 := make([][2]int, 0, 500*500)
	queue2 = append(queue2, [2]int{0, 0})
	for len(queue2) > 0 {
		point := queue2[0]
		queue2 = queue2[1:]
		i, j := point[0], point[1]
		if i > 0 && riskMap2[i-1][j] > riskMap2[i][j]+input2[i-1][j] {
			riskMap2[i-1][j] = riskMap2[i][j] + input2[i-1][j]
			queue2 = append(queue2, [2]int{i - 1, j})
		}
		if i < len(input2)-1 && riskMap2[i+1][j] > riskMap2[i][j]+input2[i+1][j] {
			riskMap2[i+1][j] = riskMap2[i][j] + input2[i+1][j]
			queue2 = append(queue2, [2]int{i + 1, j})
		}
		if j > 0 && riskMap2[i][j-1] > riskMap2[i][j]+input2[i][j-1] {
			riskMap2[i][j-1] = riskMap2[i][j] + input2[i][j-1]
			queue2 = append(queue2, [2]int{i, j - 1})
		}
		if j < len(input2[0])-1 && riskMap2[i][j+1] > riskMap2[i][j]+input2[i][j+1] {
			riskMap2[i][j+1] = riskMap2[i][j] + input2[i][j+1]
			queue2 = append(queue2, [2]int{i, j + 1})
		}
	}
	fmt.Println(riskMap2[len(riskMap2)-1][len(riskMap2[0])-1])
}
